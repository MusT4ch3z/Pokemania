...
export const dataReducer = (state = defaultState, action) => {
  switch (action.type) {
    ...
    case FETCH_ITEMS_CATEGORY_FROM_API:
      return { ...state, itemCategory: action.payload, isLoadedItems: true };
...
export const fetchPokemonDataFromApiAction = (payload) => ({
  type: FETCH_POKEMON_DATA_FROM_API,
  payload,
});
export const changeDataAction = (payload) => ({ type: CHANGE_DATA, payload });
export const fetchItemsDataFromApiAction = (payload) => ({
  type: FETCH_ITEMS_CATEGORY_FROM_API,
  payload,
});
export const changeItemsToRenderAction = (payload) => ({
  type: CHANGE_ITEMS_TO_RENDER,
  payload,
});

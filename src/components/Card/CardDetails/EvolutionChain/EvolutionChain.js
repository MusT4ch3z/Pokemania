...
const EvolutionChain = ({ info }) => {
  ...
  const fetchEvolutionChainUrl = () => {
    ...
    fetch(pokemonSpeciesUrl)
      .then((res) => res.json())
      .then((json) => {
        setEvolutionChainUrl(json.evolution_chain.url);
      })
      .catch((err) => {
        console.error("Request failed first fetch", err);
      });
  };
  ...
  const fetchEvolutionChain = () => {
    fetch(evolutionChainUrl)
      .then((res) => res.json())
      .then((json) => {
        evolutionChainPokemons.push(json.chain.species.name);
        if (json.chain.evolves_to.length === 1) {
          evolutionChainPokemons.push(json.chain.evolves_to[0].species.name);
      ...
      .catch((err) => {
        console.error("Request failed second fetch", err);
      });
  };
  ...
  useEffect(() => {
    if (evolutionChainUrl.length > 1) fetchEvolutionChain();
  }, [evolutionChainUrl]);
            ...
            <tr>
              <td>
                <div className="space_between row">
                  {evolutionChainPokemons.map((pokemonName) => (
                    <EvolutionChainItem

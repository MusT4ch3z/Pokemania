...
const CardList = () => {
  ...
  const itemsPageLimit = useSelector(
    (state) => state.paginationReducer.itemsPageLimit
  );
  ...
  const splitItems = () => {
    ...
    setDataToRender(
      data.slice(
        currentPage * itemsPageLimit - itemsPageLimit,
        currentPage * itemsPageLimit
      )
    );
  };
